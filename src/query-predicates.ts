...
export function textMatches(
  query: TextQuery,
  options?: { getIndex?: (tool: ArmorerTool) => TextSearchIndex },
): ToolPredicate {
...
export function buildTextSearchIndex(tool: ArmorerTool): TextSearchIndex {
  return {
...
export function scoreTextMatch(tool: ArmorerTool, query: TextQuery): TextMatchScore {
  const normalized = normalizeTextQuery(query);
  if (!normalized) {
...
function extractMetadataKeys(metadata: ToolMetadata | undefined): string[] {
